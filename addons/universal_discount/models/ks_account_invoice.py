...
class KsGlobalDiscountInvoice(models.Model):
            ...
            if rec.ks_global_discount_type == "amount":
                ...
                if rec.ks_global_discount_rate != 0.0:
                    rec.ks_amount_discount = (
                        rec.amount_untaxed + rec.amount_tax) * rec.ks_global_discount_rate / 100
                else:
    ...
    def ks_check_discount_value(self):
        if self.ks_global_discount_type == "percent":
            ...
            if self.ks_global_discount_rate < 0 or self.amount_untaxed < 0:
                raise ValidationError(
                    'You cannot enter discount amount greater than actual cost or lower than 0.')
    ...
    def invoice_line_move_line_get(self):
        ks_res = super(KsGlobalDiscountInvoice,
                       self).invoice_line_move_line_get()
        if self.ks_amount_discount > 0:
            ks_name = "Universal Discount"
            if self.ks_global_discount_type == "percent":
                ks_name = ks_name + \
                    " (" + str(self.ks_global_discount_rate) + "%)"
            ks_name = ks_name + " for " + \
                (self.origin if self.origin else ("Invoice No " + str(self.id)))
            if self.ks_sales_discount_account_id and (self.type == "out_invoice" or self.type == "out_refund"):
            ...
            elif self.ks_purchase_discount_account_id and (self.type == "in_invoice" or self.type == "in_refund"):
                dict = {
                    'invl_id': self.number,
                    'type': 'src',
                    'name': ks_name,
